var positiveBar = function(w, h, data, minVal, rangeRoundBands){
    ...
    .rangeRoundBands([0, height], rangeRoundBands);
    ...
    .attr("id", function(d) {
          return (d.value < minData_value ? "gradient_negative__OVERLAY" : "gradient_positive__OVERLAY")
          })
    .attr("x1", "0%")
    .attr("y1", "50%")
    .attr("x2", "100%")
    ...
    .attr("class", function(d) {
          return "bar bar--" + (d.value < minData_value ? "negative" : "positive");
          })
    .attr("x", function(d) {
    ...
    .style("fill", function (d) {
           return "url(#"+ (d.value < minData_value ? "gradient_negative__OVERLAY" : "gradient_positive__OVERLAY")+")"
           })
    ...
    .append("linearGradient")
    .attr("id", function(d) {
          return (d.value < minData_value ? "gradient_negative" : "gradient_positive")
          })
    .attr("x1", "0%")
    .attr("y1", "50%")
    .attr("x2", "100%")
    ...
    .attr("offset", "0%")
    .attr("stop-color", function(d){
          return (d.value < minData_value ? "rgb(240,146,14)" : "rgb(63,195,128)")
          })
    .attr("stop-opacity", 1);
    ...
    .attr("offset", "100%")
    .attr("stop-color", function(d){
          return (d.value < minData_value ? "rgb(245,193,58)" : "rgb(97,196,209)")
          })
    // .attr("stop-color", "#f5c13a")
    .attr("stop-opacity", 1);
    ...
    .append('rect')
    .attr('class', function(d){
          return "bar bar--" + (d.val < minData_value ? "negative" : "positive");
          })
    .attr("x", function(d) {
          return (d.val < minData_value ? - x(Math.max(d.val, 0)) : x(Math.min(0, d.val)))
          // return x(Math.min(0, d.val));
          })
    .attr("x", function(d) { return x(Math.min(d.val, 0)); })
    .attr("y", function(d) { return y(d.name); })
    .attr("width", function (d) {
          return (d.val < minData_value ? 0 : 0)
          })
    .transition()
    .duration(1500)
    .attr("width", function(d) {
          return ( d.val < minData_value ? Math.abs(x(d.val) - x(0)) : Math.abs(x(d.val) - x(0)) );
          })
    .attr("height", y.rangeBand())
    .attr("rx", y.rangeBand() / 2)
    .style("fill", function (d) {
           return "url(#"+ (d.val < minData_value ? "gradient_negative" : "gradient_positive")+")"
           })
